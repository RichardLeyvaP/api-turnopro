...
class BoxCloseController extends Controller
{
    ...
    public function box_close_newPrueba(Request $request)
    {
        DB::beginTransaction();
        try {
            Log::info("Cierre de caja Forzado");
            ...
            $bonusBranch = ProfessionalPayment::where('branch_id', $data['branch_id'])->whereDate('date', $data['data'])->where(function($query) {
                $query->where('type', 'Bono convivencias')
                    ->orWhere('type', 'Bono servicios');
            })->get();  
            
            $totalBonus = $bonusBranch->sum('amount');
            $bonus [] = $bonusBranch;
            Log::info('$totalBonus Bonussssssss');
            Log::info($totalBonus);
            Log::info($box->id);
        ...
        $mergedEmails = ['yasmany891230@gmail.com', 'pruebamal@gmaidsdl.com', 'yasmany891230@gmail.com', 'pruebamal2@gmacxvidsdl.com', 'yasmany891230@gmail.com', 'pruebamal3@gmacxvidsdl.com', 'pruebamal4@aaaaa.com', 'yasmany891230@gmail.com', 'pruebamal5@cccc.com'];
        foreach ($mergedEmails as $email) {
            try {
                $this->sendEmailService->emailBoxClosure($email, $reporte, $branch->business['name'], $branch['name'], $box['data'], $box['cashFound'], $box['existence'], $box['extraction'], $cars->sum('tip'), $products, $services, $payments->sum('cash'), $payments->sum('creditCard'), $payments->sum('debit'), $payments->sum('transfer'), $payments->sum('other'), $totalMount, $payments->sum('cardGif'), $totalBonus);
            } catch (\Swift_TransportException $e) {
    ...
    public function box_close_month(Request $request)
    {
        ...
        try {
            ...
            // Obtén el primer y el último día del mes anterior
            ...
            foreach ($branches as $branch) {
                ...
                $boxClose = BoxClose::whereHas('box', function ($query) use ($branch) {
                    $query->where('branch_id', $branch->id);
                })->whereDate('data','>=', $primerDiaMesAnterior)->whereDate('data','<=', $ultimoDiaMesAnterior)->selectRaw('
                SUM(totalMount) as totalMount,
                SUM(totalService) as totalService,
                SUM(totalProduct) as totalProduct,
                SUM(totalTip) as totalTip,
            ...
            Log::info($boxClose);
                ...
                $professionals = Professional::withoutTrashed()->whereHas('branches', function ($query) use ($branch) {
                    $query->where('branch_id', $branch->id);
                })->whereHas('charge', function ($query) {
                    $query->where('name', 'Barbero')->orWhere('name', 'Barbero y Encargado');
                })->select('id', 'name', 'surname', 'retention')->get();
                foreach ($professionals as $professional) {
                $bonusBranch = ProfessionalPayment::where('branch_id', $branch->id)->whereDate('date', "2024-11-01")->where(function($query) {
                    $query->where('type', 'Bono productos');
                })->where('professional_id', $professional->id)->first();
                if ($bonusBranch) {
                    $professionalsData[] = [
                        'name' => $professional->name,
                        'winProduct' => $bonusBranch->amount,
                    ];
                }
                 }

                $finances = Finance::Where('branch_id', $branch->id)->whereDate('data','>=', $primerDiaMesAnterior)->whereDate('data','<=', $ultimoDiaMesAnterior)->get();
                if (!$finances->isEmpty()) {
    ...
    public function box_close_automaticAnterior(Request $request)
    {
        $codigo = $request->query('codigo');  // Captura el parámetro "codigo" de la URL
        ...
        try {
            ...
            foreach ($branches as $branch) {
                ...
                if (!$box) {
                    ...
                    $box->data = $yesterday; // Aquí puedes ajustar la fecha si es necesario
